...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	Iterator<Vuelo> it1 = this.vuelos.iterator();
    	boolean found =false;
    	
    	while (it1.hasNext() && !found)
    	{
    		if (it1.next().getRuta().getCodigoRuta().equals(codigoRuta) && it1.next().getFecha().equals(fechaVuelo))
    		{
    			found=true;
    			vuelo=it1.next();
    		}
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	List<Tiquete> listaTiq= new LinkedList();
    	
    	while (it1.hasNext())
    	{
    		listaTiq.addAll(it1.next().getTiquetes());
    	}
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	Avion avion;
    	Iterator<Vuelo> it1 = this.vuelos.iterator();
    	Iterator<Avion> it2 = this.aviones.iterator();
    	
    	while (it1.hasNext() && !ocupado)
    	{
    		if(it1.next().getAvion().getNombre().equals(nombreAvion) && it1.next().getFecha().equals(fecha))
    		{
    			ocupado=true;
    		}
    	}
    	...
    	else
    	{
    		while (it2.hasNext())
    		{
    			if (it2.next().getNombre().equals(nombreAvion))
    			{
        			avion=it2.next();	
    				found=true;
     ...
     * 
     * Según la fecha del vuelo, se deben usar las tarifas de temporada baja (enero a mayo y septiembre a noviembre) o
     *  las de temporada alta (el resto del año).
     * 
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        String mes;
        String[] arrOfStr = fecha.split("-", 4);
        
        mes=arrOfStr[1];
        
        if (mes.equals("6") ||  mes.equals("7") || mes.equals("8") || mes.equals("12"))
        	calc = new CalculadoraTarifasTemporadaBaja();
        else
        	calc = new CalculadoraTarifasTemporadaAlta();
        
        if (vuelo.getAvion().getCapacidad() < cantidad+ vuelo.getTiquetes().size())
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
    	...
    	Iterator<Tiquete> it2=vuelo.getTiquetes().iterator();
    	
    	while (it2.hasNext())
    	{
    		Cliente cliente =it2.next().getCliente();
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        numSaldo=cliente.calcularValorTotalTiquetes();
        
        saldo=String.valueOf(numSaldo); 
